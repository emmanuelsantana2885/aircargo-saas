...
| `backend/aircargo-api/src/main/java/com/aircargo/service/ReceiptExportService.java` | **Rewritten** to match reference `RECIBO_DE_BODEGA_AWB.xlsx`: no borders on labels, bottom MEDIUM border on values; 10pt non-bold field labels (was bold with all THIN borders); title 10pt CENTER merged B-E (was 14pt LEFT merged B-I); table headers grey_25_percent fill (was grey_80_percent); data cols B-F have THIN borders (cols H-L no borders); 8pt row numbers; LIGHT_YELLOW fill on weight section; merged F-G checkbox labels with embedded ✓/☐; formula refs in col E (was col F); signature values bottom MEDIUM border |

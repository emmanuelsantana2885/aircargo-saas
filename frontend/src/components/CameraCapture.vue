    ...
    <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" @click.self="close">
      <div class="bg-white rounded-lg shadow-2xl overflow-hidden mx-4" style="max-width: 520px; width: 100%;">
        <div class="flex items-center justify-between px-4 py-2.5 border-b border-slate-200">
          <span class="text-[10px] font-mono font-black uppercase tracking-widest text-slate-700">Capturar Foto</span>
        ...
        <div class="relative bg-black min-h-[240px] flex items-center justify-center">
          ...
          <div v-if="!streamReady && !errorMsg" class="text-[10px] font-mono text-slate-400 uppercase tracking-widest">Iniciando cámara...</div>
          <div v-if="errorMsg" class="text-[10px] font-mono text-rose-500 px-4 text-center">{{ errorMsg }}</div>
        </div>
        <div class="flex justify-between items-center px-4 py-3 border-t border-slate-200 bg-slate-50">
          <button @click="close" class="text-[10px] px-3 py-1.5 rounded border border-slate-300 font-mono uppercase tracking-wider font-bold text-slate-600 hover:bg-white transition">
            Cancelar
          </button>
          <button v-if="streamReady" @click="capture"
            class="flex items-center gap-1.5 text-[10px] px-4 py-1.5 rounded font-mono uppercase tracking-wider font-bold text-white bg-slate-950 hover:bg-slate-800 transition active:scale-95">

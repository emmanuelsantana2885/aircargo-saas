    ...
    <header class="flex justify-between items-center border-b border-slate-400 pb-3 shrink-0">
      <div class="flex items-center gap-4">
        <div>
          ...
          <p class="text-[16px] font-mono text-slate-950 mt-0.5 uppercase tracking-widest font-bold">SDQ Control Desk</p>
        </div>
        <div class="h-8 w-[1px] bg-slate-400"></div>
        <div class="flex flex-col gap-0.5">
          <span class="text-[14px] font-black text-slate-950 uppercase tracking-widest">Vuelo</span>
        ...
        <div v-if="store.selectedFlight" class="flex gap-3 text-[15px] font-mono font-bold text-slate-950">
    ...
    <section class="flex-1 min-h-0 border border-slate-300 rounded overflow-hidden shadow-pencil-marine bg-white flex flex-col mb-1.5">
      ...
      <div v-else class="divide-y divide-slate-400 text-sm text-slate-950 overflow-y-auto flex-1 min-h-0 scrollbar-none">
        <div v-for="b in visibleBookings" :key="b.id"
          ...
          <div class="col-span-2 font-mono font-black text-slate-950 relative z-10 text-[16px]">
          ...
          <div class="col-span-2 text-slate-900 font-bold relative z-10 truncate pr-2 font-mono text-[16px] flex flex-col leading-tight">
            <span>{{ bookingReceipt(b)?.shipperName || b.shipperName || '—' }}</span>
            <span v-if="bookingReceipt(b)" class="text-[14px] text-emerald-600 font-semibold">&#10003; Recibido</span>
          </div>
          ...
          <div class="col-span-1 text-right font-mono font-bold text-slate-950 relative z-10 pr-2">
            <template v-if="bookingReceipt(b)">
              {{ Number(bookingReceipt(b).chargeableWeightKg || bookingReceipt(b).actualWeightKg || 0).toLocaleString() }}<span class="text-[15px] text-slate-950 font-normal font-mono">k</span>
            </template>
            <template v-else>
              {{ b.reservedKg ? Number(b.reservedKg).toLocaleString() : '—' }}<span class="text-[15px] text-slate-950 font-normal font-mono">k</span>
            </template>
          </div>
          ...
          <div class="col-span-2 flex items-center justify-center gap-1.5 relative z-10">
            <div class="flex items-center gap-2 text-[15px] font-mono" :title="'MAWB: ' + getMawbStatus(b)">
              ...
              <span v-if="getMawbStatus(b) !== '—'" class="text-slate-300 text-[16px]">·</span>
            </div>
          </div>
    ...
    <div v-if="showModal" class="fixed inset-0 bg-slate-950/40 backdrop-blur-sm z-50 flex items-center justify-center p-4" @click.self="closeModal">
      <div class="bg-white rounded-xl border border-slate-400 shadow-2xl w-full max-w-lg p-6">
        <div class="flex justify-between items-center mb-5 pb-3 border-b border-slate-400">
          <h2 class="text-base font-black font-mono uppercase tracking-wider text-slate-950">Nuevo Booking</h2>
    ...
    <div v-if="showImportModal" class="fixed inset-0 bg-slate-950/40 backdrop-blur-sm z-50 flex items-center justify-center p-4" @click.self="closeImportModal">
      <div class="bg-white rounded-xl border border-slate-400 shadow-2xl w-full max-w-4xl max-h-[80vh] flex flex-col">
        <div class="flex justify-between items-center px-6 py-4 border-b border-slate-400 shrink-0">
          <div>
            <h2 class="text-base font-black font-mono uppercase tracking-wider text-slate-950">Previsualización de Importación</h2>
            <p class="text-[16px] font-mono text-slate-950 mt-0.5">{{ parsedRows.length }} registros encontrados en el archivo</p>
          </div>
          <button @click="closeImportModal" class="text-slate-950 hover:text-slate-950"><IconX :size="16" :stroke-width="2" /></button>
        </div>

        <div class="overflow-auto flex-1 min-h-0">
          <table class="w-full text-[16px] font-mono">
            <thead class="bg-slate-100 sticky top-0 z-10">
              <tr class="text-[11.5px] font-black text-slate-950 uppercase tracking-wider">
            ...
            <tbody class="divide-y divide-slate-300">
              <tr v-for="(row, idx) in parsedRows" :key="idx" class="hover:bg-slate-100 transition-colors">
                ...
                <td class="px-4 py-3 text-center"><span class="inline-block text-[15px] px-1.5 py-0.5 rounded bg-slate-100 text-slate-950 font-semibold">{{ row.commodityType }}</span></td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div class="flex justify-between items-center px-6 py-4 border-t border-slate-400 bg-slate-100 rounded-b-xl shrink-0">
          <span class="text-[16px] font-mono text-slate-950">Se crearán {{ parsedRows.length }} bookings + MAWBs automáticamente</span>

      ...
      <div class="flex items-center gap-4">
        ...
        <div class="flex items-center gap-3 font-mono text-xs font-bold">
          ...
          <div class="flex items-center gap-1.5 text-slate-500">
            <span class="h-2 w-2 rounded-full bg-slate-600 animate-pulse"></span> OPERATIONAL SYSTEM LIVE
          </div>
        </div>
      </div>
    </header>
        ...
        <div v-for="uld in uldsList" :key="uld.id" class="flex flex-col">
          ...
          <div @click="toggleUldExpansion(uld.id)"
            ...
            <div class="col-span-2 font-mono font-black text-slate-950 relative z-10 flex items-center gap-1.5">
              ...
              {{ uld.id || 'NUEVO-ULD' }}
              <span v-if="!uld.sentToLoadPlanning" class="text-[7.5px] bg-slate-500/10 text-slate-700 border border-slate-300 font-bold px-1 rounded tracking-tight animate-pulse shrink-0 font-sans">LLENANDO</span>
            </div>
            ...
            <div class="col-span-1 text-center font-mono font-black relative z-10" :class="uld.volumePct >= 90 ? 'text-slate-600' : 'text-slate-950'">
              {{ uld.volumePct }}%
            </div>
          ...
          <div v-show="expandedUldId === uld.id" class="bg-slate-50 border-b border-slate-200 px-12 py-4 transition-all duration-300 overflow-hidden">
            <div class="bg-white border border-slate-300 rounded shadow-sm max-w-5xl p-6 font-mono text-sm relative">
              ...
              <div v-if="!uld.sentToLoadPlanning" class="mb-4 p-3 bg-slate-50 border border-slate-200 rounded text-slate-800 text-sm flex justify-between items-center">
                ...
                <span class="font-black tracking-widest uppercase text-[9px] bg-slate-200 px-1.5 py-0.5 rounded">MODO BORRADOR ABIERTO</span>
              </div>
                  ...
                  <div v-for="(mawb, mIdx) in uld.mawbs" :key="mIdx" class="grid grid-cols-12 items-center py-3 px-5 bg-white gap-4 text-sm">
                    ...
                    <div class="col-span-1 flex items-center justify-center gap-1">
                      <input v-model.number="mawb.percentage" type="number" min="0" max="100"
                        class="w-14 border-b border-slate-200 focus:outline-none focus:border-slate-950 py-1.5 text-center bg-transparent font-bold text-slate-600" />
                    ...
                    <div class="col-span-1 text-center">
                      <button @click="removeMawbRow(uld, mIdx)"
                        class="text-slate-500 hover:text-slate-700 font-sans text-xl leading-none">✕</button>
                    </div>
                  </div>
                </div>
              ...
              <div class="grid grid-cols-3 border border-slate-200 rounded overflow-hidden bg-slate-50 text-center divide-x divide-slate-200 mb-6">
                ...
                <div class="p-4 bg-slate-50/50 flex flex-col justify-center">
                  <span class="block text-[9px] font-black text-slate-700 uppercase tracking-wider">Cálculo de Peso Neto</span>
                  <span class="block text-sm font-black text-slate-600 mt-1">{{ ((uld.grossWeight || 0) - (uld.tareWeight || 0)).toLocaleString() }} lbs</span>
                </div>
              </div>
              ...
              <div class="border-t border-slate-200 pt-5 flex justify-end gap-2 bg-slate-50/50 -mx-6 -mb-6 p-6 rounded-b">
                <button v-if="!uld.sentToLoadPlanning" @click="dispatchToLoadPlanning(uld)"
                  class="bg-slate-800 hover:bg-slate-700 text-white font-mono font-black uppercase text-sm tracking-widest px-6 py-2.5 rounded shadow-md transition-all flex items-center gap-2">
                  🚀 Enviar a Load Planning
                </button>
                <div v-else class="flex items-center gap-1.5 text-slate-700 font-bold text-sm uppercase font-mono tracking-wider bg-slate-100/80 px-4 py-2 rounded border border-slate-300">
